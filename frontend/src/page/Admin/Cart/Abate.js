...
const Abate = () => {
    ...
    const putAbateStatus = async (id) => {
        try {
            const response = await axios.put(`http://127.0.0.1:8000/api/abate/${id}/update-status`)
            // Cập nhật trạng thái trong danh sách abates
            setAbates((prevAbates) =>
                prevAbates.map((aba) =>
                    aba.id === id ? { ...aba, abate_status: 1 } : aba
                )
            );
        } catch (error) {
